...
fit_file <- 'fit_n_71_2024-01-09_bandit2arm_delta_PH_hierarchical_estimation1_delta0.9_stepsize0.5.rds'
...
-   mu: overall group-level means of learning rate (alpha), inverse temperature (tau), and gamma (across all subjects and sessions)
...
-   **A**, **tau**, **gamma**: transformed visit-in-subject-level parameters in appropriate range (0-1 for alpha, 0-20 for tau)
...
if (estimation==1) {
  ...
  write.table(log_likelihood, file=file.path(out_path, 'log_lik_PH_hierarchical.txt'), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = F)
...
names(params) <- c("ID_block", "alpha", "tau", "gamma")
...
if (estimation==1) {
  write.table(params, file=file.path(out_path, 'Params_PH_hierarchical.txt'), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
...
if (estimation==1) {
  write.table(PE_summary_df, file=file.path(out_path, 'fmri_PEs_PH_hierarchical.txt'), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = F)
}
```
...
if (estimation==1) {
  write.table(Q_summary_df, file=file.path(out_path, 'fmri_Qs_PH_hierarchical.txt'), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = F)
}
```
...
if (estimation==1) {
  write.table(pc_summary_df, file=file.path(out_path, 'fmri_choiceprobs_PH_hierarchical.txt'), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = F)
}
```
...
if (estimation==1) {
  write.table(k_summary_df, file=file.path(out_path, 'fmri_ks_PH.txt'), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = F)
}
```
