...
fit_file <- 'fit_n56_2024-02-28_bandit2arm_delta_main_hierarchical_group_estimation1_delta0.9_stepsize0.5.rds'
...
if (sample == 'n71') {
  ...
  load(file.path(behavpath,"Input/stan_data_bandit2arm_delta_main_hierarchical_group_n56.RData")) # stan_data
} else if (sample == 'n53') {
...
if (estimation==1) {
  write.table(params, file=file.path(out_path, 'Params_main_hierarchical_group.txt'), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
...
if (estimation==1) {
  write.table(PE_summary_df, file=file.path(out_path, 'fmri_PEs_main_hierarchical_group.txt'), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
...
if (estimation==1) {
  write.table(Q_summary_df, file=file.path(out_path, 'fmri_Qs_main_hierarchical_group.txt'), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = F)
}
```
...
if (estimation==1) {
  write.table(pc_summary_df, file=file.path(out_path, 'fmri_choiceprobs_main_hierarchical_group.txt'), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = F)
}
```
