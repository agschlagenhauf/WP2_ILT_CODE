...
if (estimation==1) {
  write.table(params, file=file.path(out_path, paste('Params_PH_withC_red_hierarchical_', sample, '.txt', sep="")), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
...
```{r}
# mean across iterations of all chains
PE <- as.matrix(summary(fit, pars = "PE_chosen_pred")$summary[, c("mean")])
...
PE_jui_summary_df <- PE_jui_summary_df %>%
  rename(PE_chosen_pred = "c(t(PE_jui_summary))") %>%
  mutate_at(c('PE_chosen_pred'), ~na_if(.,-999))
PE_jui_summary_df$absPE_chosen_pred <- abs(PE_jui_summary_df$PE_chosen_pred)
...
PE_alc_summary_df <- PE_alc_summary_df %>%
  rename(PE_chosen_pred = "c(t(PE_alc_summary))") %>%
  mutate_at(c('PE_chosen_pred'), ~na_if(.,-999))
PE_alc_summary_df$absPE_chosen_pred <- abs(PE_alc_summary_df$PE_chosen_pred)
...
if (estimation==1) {
  write.table(PE_summary_df, file=file.path(out_path, paste('fmri_PEs_PH_withC_hierarchical_', sample, '.txt', sep = "")), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
...
PE_summary_per_trial <- PE_summary_df %>%
  ...
  summarise_at(vars(absPE_chosen_pred), list(PE_mean = mean), na.rm=T)
...
if (estimation==1) {
  write.table(Q_summary_df, file=file.path(out_path, paste('fmri_Qs_PH_withC_hierarchical_', sample, '.txt', sep = "")), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
...
if (estimation==1) {
  write.table(pc_summary_df, file=file.path(out_path, paste('fmri_choiceprobs_PH_withC_hierarchical_', sample, '.txt', sep = "")), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
...
if (estimation==1) {
  write.table(k_summary_df, file=file.path(out_path, paste('fmri_ks_PH_withC:', sample, '.txt', sep = "")), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
