...
fit_file <- 'fit_n56_hc_alc_2024-02-26_bandit2arm_delta_PH_withC_DU_onegamma_oneC_init05_estimation1_delta0.8_stepsize0.5.rds'
...
traceplot(fit, pars=c("mu_pr[3]", "mu_gamma"),inc_warmup=T)

traceplot(fit, pars=c("mu_pr[4]", "mu_C"),inc_warmup=T)
```
...
```{r}
# mean across iterations of all chains
gamma_summary <- as.matrix(summary(fit, pars = "mu_gamma")$summary[, c("mean")]) 
```
...
```{r}
# mean across iterations of all chains
C_summary <- as.matrix(summary(fit, pars = "mu_C")$summary[, c("mean")]) 
```
...
params$gamma <- gamma_summary[1,1]
params$C <- C_summary[1,1]
...
if (estimation==1) {
  write.table(params, file=file.path(out_path, paste('Params_PH_withC_DU_onegamma_oneC_', sample, '.txt', sep="")), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
...
if (estimation==1) {
  write.table(PE_summary, file=file.path(out_path, paste('fmri_PEs_PH_withC_DU_onegamma_oneC_', sample, '.txt', sep="")), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
...
if (estimation==1) {
  write.table(Q_summary, file=file.path(out_path, paste('fmri_Qs_PH_withC_DU_onegamma_oneC_', sample, '.txt', sep="")), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
...
if (estimation==1) {
  write.table(pc_summary, file=file.path(out_path, paste('fmri_pcs_PH_withC_DU_onegamma_oneC_', sample, '.txt', sep="")), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
