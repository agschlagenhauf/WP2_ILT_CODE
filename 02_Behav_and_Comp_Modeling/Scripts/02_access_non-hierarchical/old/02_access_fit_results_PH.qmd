...
fit_file <- 'fit_n56_aud_alc_2024-02-23_bandit2arm_delta_PH_estimation1_delta0.8_stepsize0.5.rds'
...
if (estimation==1) {
  write.table(params, file=file.path(out_path, paste('Params_PH_', sample, '.txt', sep="")), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
...
if (estimation==1) {
  write.table(PE_summary, file=file.path(out_path, paste('fmri_PEs_PH_', sample, '.txt', sep="")), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
...
if (estimation==1) {
  write.table(Q_summary, file=file.path(out_path, paste('fmri_Qs_PH_', sample, '.txt', sep="")), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
...
if (estimation==1) {
  write.table(pc_summary, file=file.path(out_path, paste('fmri_pcs_PH_', sample, '.txt', sep="")), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
