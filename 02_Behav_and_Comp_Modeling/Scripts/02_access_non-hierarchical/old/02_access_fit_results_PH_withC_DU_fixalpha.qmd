...
fit_file <- 'fit_n56_aud_alc_2024-02-23_bandit2arm_delta_PH_withC_DU_fixalpha_estimation1_delta0.8_stepsize0.5.rds'
...
if (estimation==1) {
  ...
  parameter_names <- c("mu_pr[1]", "mu_tau", "sigma[1]", "tau_pr[20]", "tau[20]",
                       "mu_pr[2]", "mu_gamma", "sigma[2]", "gamma_pr[20]", "gamma[20]",
...
```{r}
traceplot(fit, pars=c("mu_pr[1]", "mu_tau", "sigma[1]", "tau_pr[20]", "tau[20]"),inc_warmup=T)

traceplot(fit, pars=c("mu_pr[2]", "mu_gamma", "sigma[2]", "gamma_pr[20]", "gamma[20]"),inc_warmup=T)
...
```{r}
stan_hist(fit, pars=c("mu_pr[1]", "mu_tau", "sigma[1]", "tau_pr[20]", "tau[20]",
                       "mu_pr[2]", "mu_gamma", "sigma[2]", "gamma_pr[20]", "gamma[20]",
                       "mu_pr[3]", "mu_C", "sigma[3]", "C_pr[20]", "C[20]"))
```
...
```{r}
plot(fit, pars=c("mu_pr[1]", "mu_tau", "sigma[1]", "tau_pr[20]", "tau[20]",
                       "mu_pr[2]", "mu_gamma", "sigma[2]", "gamma_pr[20]", "gamma[20]",
                       "mu_pr[3]", "mu_C", "sigma[3]", "C_pr[20]", "C[20]"), 
...
if (estimation==1) {
  write.table(params, file=file.path(out_path, paste('Params_PH_withC_DU_fixalpha_', sample, '.txt', sep="")), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
...
if (estimation==1) {
  write.table(PE_summary, file=file.path(out_path, paste('fmri_PEs_PH_withC_DU_fixalpha_', sample, '.txt', sep="")), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
...
if (estimation==1) {
  write.table(Q_summary, file=file.path(out_path, paste('fmri_Qs_PH_withC_DU_fixalpha_', sample, '.txt', sep="")), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
...
if (estimation==1) {
  write.table(pc_summary, file=file.path(out_path, paste('fmri_pcs_PH_withC_DU_fixalpha_', sample, '.txt', sep="")), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = T)
}
```
