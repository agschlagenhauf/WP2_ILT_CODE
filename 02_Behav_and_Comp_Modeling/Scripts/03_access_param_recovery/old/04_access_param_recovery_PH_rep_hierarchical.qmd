...
# load draws
draws <- c(read.table(file.path(out_path, 'Parameter_Recovery/draws_bandit2arm_delta_PH_rep_hierarchical.txt'), sep = " ")[1:100,1]) # adapt nrows in case of more simulation runs

# get file names for simulated data results
sim_fit_files <- list.files(file.path(out_path, 'Parameter_Recovery'), pattern = "*.bandit2arm_delta_PH_rep_hierarchical.*\\.rds$", full.names = F)
...
```{r}
params <- data.frame(true_A_all,true_tau_all,true_gamma_all,true_pi_all,sim_A_all,sim_tau_all,sim_gamma_all,sim_pi_all)
cor(params)
pairs(params)
```
