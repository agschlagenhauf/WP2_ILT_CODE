...
parameters {
  ...
  vector[3] mu; // group level means for the 3 parameters
  ...
  //cross-level interaction effects (fixed interaction effect)
...
transformed parameters {
  ...
  matrix[N,C] pi_normal; // pi without range
  ...
  for (s in 1:N) {
    ...
    pi_normal[s,] = mu[3] + pi_visit_s*pi_visit_raw[s,] + pi_vars[s,1];
    ...
    for (s in 1:N) {
      ...
      for (v in 1:C) {
        ...
        for (t in 1:Tsubj[s,v]) {
          ...
          ev[choice[s,t,v]+1] += A[s,v] * PE; // Q + alpha * PE                                                                                       
