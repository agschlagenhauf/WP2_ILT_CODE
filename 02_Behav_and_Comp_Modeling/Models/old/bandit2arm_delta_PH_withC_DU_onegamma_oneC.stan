...
transformed parameters {
  ...
  real<lower=0, upper=1> mu_gamma; // bring gamma to range between...
                                      // 0 and 1 (as in Theresas code)
  real<lower=0, upper=1> mu_C; // arbitrary constant                                    
  ...
  if (run_estimation==1){
    ...
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        k = mu_gamma*mu_C*absPE_chosen + (1-mu_gamma)*k; // decay constant * arbitrary constant * absolute PE from last trial + (1-decay constant) * learning rate from last trial
    ...
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        if (run_estimation==1){
          ...
          // Pearce Hall learning rate
          k = mu_gamma*mu_C*absPE_chosen + (1-mu_gamma)*k;
          k_pred[i,t] = k;
