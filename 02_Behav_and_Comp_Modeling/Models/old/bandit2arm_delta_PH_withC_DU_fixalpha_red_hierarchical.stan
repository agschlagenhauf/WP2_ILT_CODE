...
parameters {
  ...
  vector[3] mu; // fixed intercepts for the 4 parameters, for HC and juice (these are coded as 0)
...
transformed parameters {
  ...
  for (s in 1:N) {
    ...
    for (v in 1:C) { // for every condition
      ...
      tau_normal[s,v] = mu[1] + tau_vars[s]; // fixed intercept + random intercept per subject
      gamma_normal[s,v] = mu[2] + gamma_vars[s];
      C_normal[s,v] = mu[3] + C_vars[s];
      
      for (kv in 1:kV) { 
        //fixed effects of visit-level variables
        tau_normal[s,v] += visit_vars[s,v,kv]*tau_sub_con_m[kv]; // predictor * fixed slope
        ...
        for (ks in 1:kS) { 
          ...
          tau_normal[s,v] += subj_vars[s,ks]*tau_sub_m[ks]; // predictor * fixed slope
          gamma_normal[s,v] += subj_vars[s,ks]*gamma_sub_m[ks];
          C_normal[s,v] += subj_vars[s,ks]*C_sub_m[ks];
          
          //fixed cross-level interactions
          tau_normal[s,v] += subj_vars[s,ks]*visit_vars[s,v,kv]*tau_int_m[ks,kv]; // predictor * predictor * fixed slope
...
generated quantities {
  ...
  mu_tau = Phi_approx(mu[1]) * 100;
  mu_gamma   = Phi_approx(mu[2]);
  mu_C   = Phi_approx(mu[3]);
