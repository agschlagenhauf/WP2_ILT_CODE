...
data {
  ...
  int<lower=1> C; // number of visits (reinforcer_type)
...
transformed parameters {
  ...
  for (s in 1:N) {
    A_normal[s,]   = mu[1] + A_visit_s*A_visit_raw[s,] + A_vars[s,1]; // fixed intercept + random intercept per visit + random intercept per subject
    ...
    for (v in 1:C) { // for every condition
      ...
      for (kv in 1:kV) { 
        ...
        A_normal[s,v] += visit_vars[s,v,kv]*(A_sub_con_m[kv]+A_vars[s,kv+1]); // predictor * (fixed slope + random slope of visit-level variable nested within subjects)
