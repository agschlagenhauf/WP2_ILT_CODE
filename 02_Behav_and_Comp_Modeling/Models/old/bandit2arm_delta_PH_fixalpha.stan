  ...
  if (run_estimation==1){
    ...
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        k = gamma[i]*absPE + (1-gamma[i])*k; // decay constant * arbitrary constant * absolute PE from last trial + (1-decay constant) * learning rate from last trial
