...
transformed data {
  ...
  initV = rep_vector(0, 2); // set initial Q value to 0.5 (middle btw. 0 and 1)
...
// Declare all parameters as vectors for vectorizing
  ...
  vector[2] mu_pr; // group level means for the 4 parameters
  vector<lower=0>[2] sigma; // group level variance for the 4 parameters
  ...
  if (run_estimation==1){
    ...
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        k = gamma[i]*absPE_chosen + (1-gamma[i])*k; // decay constant * arbitrary constant * absolute PE from last trial + (1-decay constant) * learning rate from last trial
    ...
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        if (run_estimation==1){
          ...
          // Pearce Hall learning rate
          k = gamma[i]*absPE_chosen + (1-gamma[i])*k;
          k_pred[i,t] = k;
