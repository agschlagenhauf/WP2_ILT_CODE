...
parameters {
  ...
  vector[6] mu; // fixed intercepts for the 4 parameters, for HC and juice (these are coded as 0)
  ...
  vector[kS] omega_sub_m; // arbitrary constant
  vector[kS] initV_sub_m; // arbitrary constant
  ...
  vector[kV] omega_sub_con_m;  // arbitrary constant
  vector[kV] initV_sub_con_m;  // arbitrary constant
  ...
  matrix[kV,kS] omega_int_m;
  matrix[kV,kS] initV_int_m;
  ...
  real<lower=0> omega_subj_s;
  real<lower=0> initV_subj_s;
...
transformed parameters {
  ...
  matrix<lower=0, upper=1>[N,C] initV; // bring initV to range between 0 and 1
  matrix[N,C] initV_normal; // initial balue of both cards
  ...
  vector[N] initV_vars = initV_subj_s*initV_subj_raw;
  ...
  for (s in 1:N) {
    ...
    for (v in 1:C) { // for every condition
      ...
      initV_normal[s,v] = mu[5] + initV_vars[s];
      ...
      for (kv in 1:kV) { 
        ...
        initV_normal[s,v] += visit_vars[s,v,kv]*initV_sub_con_m[kv];
        ...
        for (ks in 1:kS) { 
          ...
          initV_normal[s,v] += subj_vars[s,ks]*initV_sub_m[ks];
          ...
          initV_normal[s,v] += subj_vars[s,ks]*visit_vars[s,v,kv]*initV_int_m[ks,kv];
    ...
    initV[s,] = Phi_approx(initV_normal[s,]);
