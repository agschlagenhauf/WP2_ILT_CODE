  ...
  if (run_estimation==1){
    ...
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        k = initG*absPE_chosen + (1-initG)*k; // decay constant * arbitrary constant * absolute PE from last trial + (1-decay constant) * learning rate from last trial
    ...
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        if (run_estimation==1){
          ...
          // Pearce Hall learning rate
          k = initG*absPE_chosen + (1-initG)*k;
          k_pred[i,t] = k;
