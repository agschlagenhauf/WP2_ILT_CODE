  ...
  if (run_estimation==1){
    ...
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        k = gamma[i]*mu_C*absPE_chosen + (1-gamma[i])*k; // decay constant * arbitrary constant * absolute PE from last trial + (1-decay constant) * learning rate from last trial
    ...
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        if (run_estimation==1){
          ...
          // Pearce Hall learning rate
          k = gamma[i]*mu_C*absPE_chosen + (1-gamma[i])*k;
          k_pred[i,t] = k;
