...
transformed parameters {
  ...
  real<lower=0, upper=1> mu_A; // bring alpha to range between 0 and 1
  ...
  for (i in 1:N) { // subject loop
    ...
    tau[i] = Phi_approx(mu_pr[2] + sigma[1] * tau_pr[i]) * 100;
    gamma[i] = Phi_approx(mu_pr[3] + sigma[2] * gamma_pr[i]);
    C[i] = Phi_approx(mu_pr[4]  + sigma[3]  * C_pr[i]); // times 10 as same done for tau?
  ...
  if (run_estimation==1){
    ...
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        k = gamma[i]*C[i]*absPE_chosen + (1-gamma[i])*k; // decay constant * arbitrary constant * absolute PE from last trial + (1-decay constant) * learning rate from last trial
    ...
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        if (run_estimation==1){
          ...
          // Pearce Hall learning rate
          k = gamma[i]*C[i]*absPE_chosen + (1-gamma[i])*k;
          k_pred[i,t] = k;
