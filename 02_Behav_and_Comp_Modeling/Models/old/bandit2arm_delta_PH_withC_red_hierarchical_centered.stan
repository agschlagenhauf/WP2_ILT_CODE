...
model {
  ...
  A_sub_m ~ normal(0, 0.5);
  tau_sub_m ~ normal(0, 0.5);
  gamma_sub_m ~ normal(0, 0.5);
  C_sub_m ~ normal(0, 0.5);
  
  // Condition-level raw parameters
  A_sub_con_m ~ normal(0,0.5);
  tau_sub_con_m ~ normal(0,0.5);
  gamma_sub_con_m ~ normal(0,0.5);
  C_sub_con_m ~ normal(0,0.5);
  
  // cross-level interactions
  for (ks in 1:kS) {
    A_int_m[,ks] ~ normal(0,0.5);
    tau_int_m[,ks] ~ normal(0,0.5);
    gamma_int_m[,ks] ~ normal(0,0.5);
    C_int_m[,ks] ~ normal(0,0.5);
  }
  
  //SDs of visit-level effects across subjects
  A_subj_s ~ cauchy(0,1);
  tau_subj_s ~ cauchy(0,1);
  gamma_subj_s ~ cauchy(0,1);
  C_subj_s ~ cauchy(0,1);
