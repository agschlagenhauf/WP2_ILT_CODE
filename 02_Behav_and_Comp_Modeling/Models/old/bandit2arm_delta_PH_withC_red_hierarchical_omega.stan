...
parameters {
  ...
  vector[5] mu; // fixed intercepts for the 4 parameters, for HC and juice (these are coded as 0)
  ...
  vector[kS] omega_sub_m; // reward scaling
  ...
  vector[kV] omega_sub_con_m;  // reward scaling
...
transformed parameters {
  ...
  matrix<lower=0, upper=1>[N,C] omega; // bring omega to range between 0 and 1
  matrix[N,C] omega_normal; // omega without range
  ...
  vector[N] omega_vars = omega_subj_s*omega_subj_raw;
  ...
  for (s in 1:N) {
    ...
    for (v in 1:C) { // for every condition
      ...
      omega_normal[s,v] = mu[5] + omega_vars[s];
      ...
      for (kv in 1:kV) { 
        ...
        omega_normal[s,v] += visit_vars[s,v,kv]*omega_sub_con_m[kv];
        ...
        for (ks in 1:kS) { 
          ...
          omega_normal[s,v] += subj_vars[s,ks]*omega_sub_m[ks];
          ...
          omega_normal[s,v] += subj_vars[s,ks]*visit_vars[s,v,kv]*omega_int_m[ks,kv];
        }
    ...
    omega[s,] = Phi_approx(omega_normal[s,]);
    ...
    for (s in 1:N) {
      ...
      for (v in 1:C) {
        ...
        for (t in 1:Tsubj[s,v]) {
          ...
          // prediction error
          PE = omega[s,v] * outcome[s,t,v] - ev[choice[s,t,v]+1]; // outcome - Q of choice taken
          absPE = abs(PE);
      ...
      for (v in 1:C) {
        ...
        for (t in 1:Tsubj[s,v]) {
          ...
          if (run_estimation==1){
            ...
            // prediction error
            PE = omega[s,v] * outcome[s,t,v] - ev[choice[s,t,v]+1];
            PE_pred[s,t,v] = PE;
          ...
          if (run_estimation==0){
            ...
            // prediction error
            PE = omega[s,v] * outcome[s,t,v] - ev[y_pred[s,t,v]+1];
            PE_pred[s,t,v] = PE;
