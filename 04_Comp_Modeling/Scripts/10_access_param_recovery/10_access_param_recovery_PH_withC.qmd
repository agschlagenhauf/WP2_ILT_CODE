...
datapath <- '/data/cephfs-2/unmirrored/groups/schlagenhauf/B01_FP1_WP2/WP2_ILT_CODE/02_Behav_and_Comp_Modeling/'
out_path <- '/data/cephfs-2/unmirrored/groups/schlagenhauf/B01_FP1_WP2/WP2_ILT_CODE/02_Behav_and_Comp_Modeling/Output'
behavpath <- '/data/cephfs-2/unmirrored/groups/schlagenhauf/B01_FP1_WP2/ILT_DATA'

# load files containing true parameters used as input for simulation
orig_file <- 'fit_n58_2024-05-07_bandit2arm_delta_PH_withC_estimation1_delta0.999_stepsize0.1_treedepth12.rds'
...
# True parameters in 95% interval?

```{r}
# extract intervals
A_intervals <- mcmc_intervals_data(recovered_A, prob = 0.5, prob_outer = 0.95)
tau_intervals <- mcmc_intervals_data(recovered_tau, prob = 0.5, prob_outer = 0.95)
gamma_intervals <- mcmc_intervals_data(recovered_gamma, prob = 0.5, prob_outer = 0.95)
C_intervals <- mcmc_intervals_data(recovered_C, prob = 0.5, prob_outer = 0.95)

# get percent true parameters within 95% credible interval
A_intervals$true_params <- true_A
A_recoverability <- A_intervals %>%
  mutate(within_interval = (true_params >= ll & true_params <= hh)) %>%
  summarize(percent_recovered = mean(within_interval) * 100)
print(paste("A recoverability:", A_recoverability))

tau_intervals$true_params <- true_tau
tau_recoverability <- tau_intervals %>%
  mutate(within_interval = (true_params >= ll & true_params <= hh)) %>%
  summarize(percent_recovered = mean(within_interval) * 100)
print(paste("tau recoverability:", tau_recoverability))

gamma_intervals$true_params <- true_gamma
gamma_recoverability <- gamma_intervals %>%
  mutate(within_interval = (true_params >= ll & true_params <= hh)) %>%
  summarize(percent_recovered = mean(within_interval) * 100)
print(paste("gamma recoverability:", gamma_recoverability))

C_intervals$true_params <- true_C
C_recoverability <- C_intervals %>%
  mutate(within_interval = (true_params >= ll & true_params <= hh)) %>%
  summarize(percent_recovered = mean(within_interval) * 100)
print(paste("C recoverability:", C_recoverability))
```
