...
% define output path
stats_path = 'S:\AG\AG-Schlagenhauf_TRR265\Daten\B01\WP2_DATA\derivatives\02_ILT\00_spm12_1st_level\PH_withC_n58_constant_feedback_+_sensoric_correctbaseline_compcor_correctPC_posnegPE_on_trial'; % folder for single stats of this specific model, add names of pmods
...
for n = 1:length(ids)
    ...
    for block = 1:length(blocks)
        ...
        PEs{n,block} = PEs_sub; % do not mean-center RPE as we will split
        ...
        PEs_pos{n,block} = PEs{n,block};
        PEs_neg{n,block} = PEs{n,block};

        PEs_pos{n,block}(PEs_pos{n,block}<0) = 0;
        PEs_neg{n,block}(PEs_neg{n,block}>0) = 0;
        
        PEs_neg{n,block} = PEs_neg{n,block}*-1;

        regs          = [PEs_pos{n,block} PEs_neg{n,block} PCs{n,block}];
        ...
        for i=1:length(onsets_cue)
            ...
            % == ones for every cue bin, twos for every feedback bins
            ...
            PE_pos_modulator(ind(i)+1:ind(i+1),1)=[zeros(cue_events(i),1); repmat(regs(i,1), feedback_events(i), 1)]; 
            ...
            % vector of length feedback_events (containing PE value of
            % trial) and with zeros vector of length swallow events
            PE_neg_modulator(ind(i)+1:ind(i+1),1)=[zeros(cue_events(i),1); repmat(regs(i,2), feedback_events(i), 1)]; 
            ...
            if     i< length(onsets_cue)
                PC_modulator(ind(i)+1:ind(i+1),1) = [repmat(regs(i,3), cue_events(i), 1); repmat(regs(i+1,3), feedback_events(i), 1)];
            elseif i==length(onsets_cue) % letzter Trial
                PC_modulator(ind(i)+1:ind(i+1),1) = [repmat(regs(i,3), cue_events(i), 1); repmat(regs(i,3), feedback_events(i), 1)];
            end
        end
        ...
        orth = {0 0 0}; % 0 = beide können gleiche Varianz erklären
        
        pmod(1).name{1} = 'prediction_error_pos';
        pmod(1).param{1}= [PE_pos_modulator(:,1)];
        pmod(1).poly{1} = 1;

        pmod(1).name{2} = 'prediction_error_neg';
        pmod(1).param{2}= [PE_neg_modulator(:,1)];
        pmod(1).poly{2} = 1;

        pmod(1).name{3} = 'choice_prob';
        pmod(1).param{3}= [PC_modulator(:,1)];
        pmod(1).poly{3} = 1;
        ...
        clear regs pmod onsets names durations onsets_cue onsets_cue_missings onsets_feedback onsets_feedback_missings onsets_taste onsets_swallow onsets_trialend onsets_trialend_missings orth ons ons_m ons_s PE_pos_modulator PE_neg_modulator PC_modulator trial_length feedback_length cue_length swallow_length missing_trial_length
