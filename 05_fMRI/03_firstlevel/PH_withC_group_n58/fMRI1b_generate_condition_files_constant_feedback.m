...
stats_path = 'S:\AG\AG-Schlagenhauf_TRR265\Daten\B01\WP2_DATA\derivatives\02_spm12_1st_level\PH_withC_group_n58_constant_feedback'; % folder for single stats of this specific model, add names of pmods
...
%         end
        ...
        names           = {'trial', 'swallow', 'missing_trial'};
        durations       = {0 0 0};
        ...
        for trial = 1:length(onsets_cue)
            if onsets_taste(trial,1) < 0 % if NO taste & swallow in this trial
                ...
                swallow_length(trial,1) = onsets_trialend(trial,1)-onsets_swallow(trial,1); % swallow length
            end
        end
        ...
        % get number of bins per swallow phase
        swallow_events_c = ceil(swallow_length/bin); % nbins per trial with one incomplete bin
        swallow_events_f = floor(swallow_length/bin); % only full bins
        swallow_diff_c = abs(swallow_length-swallow_events_c*bin); % diff btw ceil*bin and trial length
        swallow_diff_f = abs(swallow_length-swallow_events_f*bin); % diff btw floor*bin and trial length

        if swallow_diff_c < swallow_diff_f % check which is closer to actual trial length and choose this as events variable
            swallow_events = swallow_events_c;
        else
            swallow_events = swallow_events_f;
        end
        
        swallow_events = swallow_events(swallow_events>0); % exclude trials with no swallow phase
        ...
        % swallow phase
        ind_s = [0; cumsum(swallow_events(1:end))]; % past bins at the beginning of each swallow phase
        for s=1:length(onsets_swallow)
            % onsets matrix in lines from number of past bins at trial start + 1
            % to number of past bins at next trial start, first column
            % == numbers starting from cue onset of trial, going in bin
            % steps, until cue onset of trial + (number of bins in that
            % trial - 1) * bin
            ons_s(ind_s(s)+1:ind_s(s+1),1)=[onsets_swallow(s):bin:(onsets_swallow(s)+(swallow_events(s)-1)*bin)]'; 
        end
