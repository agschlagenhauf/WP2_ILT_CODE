...
% define output path
stats_path = 'S:\AG\AG-Schlagenhauf_TRR265\Daten\B01\WP2_DATA\derivatives\02_ILT\00_spm12_1st_level\PH_withC_n58_constant_feedback_+_sensoric_correctbaseline_compcor_correctPC_winnowin_Q'; % folder for single stats of this specific model, add names of pmods
...
Qs = readtable(fullfile(stats_path, 'fmri_Qs_PH_withC_init05_n58.txt'));
...
for n = 1:length(ids)
    ...
    for block = 1:length(blocks)
        ...
        if block == 1
            D_sub = D_ilt1;
            Qs_sub = subj_Qs_block1;
            win_nowin_sub = (D_sub.R+1)/2;
        elseif block == 2
            D_sub = D_ilt2;
            Qs_sub = subj_Qs_block2;
            win_nowin_sub = (D_sub.R+1)/2;
        end
                
        % define regressors for fMRI analysis
        Qs{n,block} = Qs_sub-nanmean(Qs_sub);
        win_nowin{n,block} = win_nowin_sub'; % do not mean-center win nowin to keep it interpretable

        Qs{n,block} = Qs{n,block}(isnan(Qs{n,block})==0); % exclude NaNs from pmods
        win_nowin{n,block} = win_nowin{n,block}(isnan(win_nowin{n,block})==0);

        regs          = [Qs{n,block} win_nowin{n,block}];
        ...
        for i=1:length(onsets_cue)
            ...
            % feedback_events (still containing old PC value)
            Q_modulator(ind(i)+1:ind(i+1),1) = [repmat(regs(i,1), cue_events(i), 1); repmat(regs(i,1), feedback_events(i), 1)];

            % win nowin modulator
            % parametric modulator matrix in lines from number of past bins 
            % at trial start + 1 to number of past bins at next trial
            % start, column 1
            % == zero vector of length cue_events concatenated with win
            % nowin vector of length feedback_events
            win_nowin_modulator(ind(i)+1:ind(i+1),1)=[zeros(cue_events(i),1); repmat(regs(i,2), feedback_events(i), 1)]; 
        end
        ...
        pmod(1).name{1} = 'q_value';
        pmod(1).param{1}= [Q_modulator(:,1)];
        pmod(1).poly{1} = 1;

        pmod(1).name{2} = 'win_nowin';
        pmod(1).param{2}= [win_nowin_modulator(:,1)];
        pmod(1).poly{2} = 1;
        ...
        clear regs pmod onsets names durations onsets_cue onsets_cue_missings onsets_feedback onsets_feedback_missings onsets_taste onsets_swallow onsets_trialend onsets_trialend_missings orth ons ons_m ons_s Q_modulator win_nowin_modulator trial_length feedback_length cue_length swallow_length missing_trial_length
